...
import type { InfluencerOutreachStatus } from "@/types";

interface InfluencerOutreachControlsProps {
  influencerId: string;
  initialStatus: InfluencerOutreachStatus | null;
  initialNotes: string | null;
}

const STATUS_OPTIONS: Array<{ value: InfluencerOutreachStatus; label: string }> = [
...
export function InfluencerOutreachControls({
  influencerId,
  initialStatus,
  initialNotes,
}: InfluencerOutreachControlsProps) {
  const router = useRouter();
  const [status, setStatus] = useState<InfluencerOutreachStatus>(initialStatus ?? "not_contacted");
  ...
  function save() {
    ...
    startTransition(async () => {
      const response = await fetch("/api/influencers/status", {
        method: "PATCH",
        headers: {
  ...
  return (
    <div className="mt-6 rounded-card border border-white/10 bg-white/5 p-4">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <select
          value={status}
          onChange={(event) => setStatus(event.target.value as InfluencerOutreachStatus)}
