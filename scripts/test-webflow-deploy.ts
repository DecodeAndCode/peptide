...
function makeDryRunIntegrationsStub() {
  ...
  return {
    getWebflowIntegration: async () => null,
    ...
    updateWebflowSiteConfig: async () => undefined,
  };
}

if (!moduleProto.__webflow_patched) {
  ...
  moduleProto.__webflow_patched = true;
}
...
async function main() {
  ...
  console.error(`[test-webflow-deploy] mode=${dryRun ? "dry-run" : "live"}`);

  if (dryRun) {
    const [{ deployCycleToWebflow }] = await Promise.all([
      import("@/lib/webflow/cms-deployer"),
    ]);
    ...
    const content: GeneratedContentRecord[] = [
      ...
      },
      {
        ...
        title: "Best peptide stack for recovery",
        body: "Our top recommendation for athletic recovery.\n\nDosage details inside.",
        target_prompts: [],
        medical_sources: [],
        created_at: new Date().toISOString(),
      },
  ...
  const [{ deployCycleToWebflow }, { createServiceRoleClient }] = await Promise.all([
    import("@/lib/webflow/cms-deployer"),
...
main().catch((error) => {
  console.error("[test-webflow-deploy] failed:", error);
  process.exit(1);
});
