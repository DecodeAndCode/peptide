...
import type { BrandRecord, PartnerOutreachStatus } from "@/types";
...
export async function PATCH(request: Request) {
  ...
  const { partnerId, outreachStatus, outreachNotes } = parsed.data;
  ...
  if (!existing) {
    return NextResponse.json({ error: "Partner match not found." }, { status: 404 });
  }

  const statusChangedToContactStep = (value: PartnerOutreachStatus) =>
